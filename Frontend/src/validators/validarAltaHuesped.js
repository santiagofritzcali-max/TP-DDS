export const validarAltaHuesped = (form) => {
  ...
  if (!form.fechaNacimiento) { errs.fechaNacimiento = 'El campo Fecha de nacimiento es requerido'; }
    ...
    if (Number.isNaN(fn.getTime())) {
      ...
      fn.setHours(0, 0, 0, 0);
      if (fn > hoy) {
        errs.fechaNacimiento = 'La fecha de nacimiento no puede ser posterior a hoy';
      }
    }
  }
  ...
  else if (!soloLetras.test(form.ocupacion)) { errs.ocupacion = "La ocupación solo puede contener letras"; }
  ...
  // Departamento
  if (form.direccion.departamento.trim() && !/^[A-Za-zÁÉÍÓÚáéíóúñÑüÜ\\s]+$/.test(form.direccion.departamento.trim())) { errs["direccion.departamento"] = "El campo Departamento solo puede contener letras y espacios"; }
